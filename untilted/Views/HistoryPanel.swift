        ...
        Group {
            if appState.savingsPlans.isEmpty {
                ...
                ForEach(appState.savingsPlans.sorted(by: { $0.createdAt > $1.createdAt })) { plan in
                    SavingsPlanCard(plan: plan, texts: t, currency: userData.currency)
                }
            }
        }
        .padding(.top, 16)
        ...
        Group {
            if appState.purchaseDecisions.isEmpty {
                ...
                ForEach(appState.purchaseDecisions.sorted(by: { $0.createdAt > $1.createdAt })) { decision in
                    PurchaseDecisionCard(decision: decision, texts: t, currency: userData.currency)
                }
            }
        }
        .padding(.top, 16)
...
struct SavingsPlanCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if !plan.isCompleted {
                ProgressView(value: plan.currentSavings, total: plan.targetAmount)
                    .progressViewStyle(CustomProgressViewStyle(color: .mediumGreen))
            }
        }
        .appCardStyle()
        .padding(16)
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack {
                ...
                VStack(alignment: .trailing, spacing: 4) {
                    ...
                    DecisionBadge(decision: decision.decision, texts: texts)
                }
            }
